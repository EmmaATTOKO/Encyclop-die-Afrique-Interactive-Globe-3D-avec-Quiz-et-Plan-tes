// Liste complète des pays africains (identique à la version précédente)
const countries = [
  {
    id: "algeria",
    name: "Algérie",
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  countries.forEach((country) => {
    ...
    marker.setAttribute("radius", "0.028");
    ...
    border.setAttribute("radius-inner", "0.032");
    border.setAttribute("radius-outer", "0.042");
    ...
    halo.setAttribute("radius", "0.065");
...
// Composant de rotation par glissement de souris
...
// Nouveau composant : rotation par touches clavier (flèches / ZQSD)
AFRAME.registerComponent("keyboard-rotate", {
  init: function () {
    const el = this.el;
    const speed = 1.5; // degrés par frame (environ 60 fps → 90°/s)
    const keys = {
      ArrowLeft: false,
      ArrowRight: false,
      ArrowUp: false,
    ...
    this.tick = function () {
      ...
      let deltaY = 0;
      let deltaX = 0;
      if (keys.ArrowLeft || keys.KeyA) deltaY = -speed;
      if (keys.ArrowRight || keys.KeyD) deltaY = speed;
      if (keys.ArrowUp || keys.KeyW) deltaX = -speed;
      if (keys.ArrowDown || keys.KeyS) deltaX = speed;
...
// Composant des planètes en orbite (inchangé)
